...
void main() {
  ...
  SingboxConfigBuilder createBuilder({
    ...
    return SingboxConfigBuilder(
      ...
      russiaGeositeRuBlockedPath: useRussiaRouteData ? '/tmp/geosite-ru-blocked.srs' : null,
  ...
  test('Test 1: dns.servers contains dns-fakeip with IPv4 and conditional IPv6', () {
    final configIpv4 = createBuilder(
      dnsFakeIpEnabled: true,
      dnsPreferIpv6: false,
    ).build();

    final serversIpv4 = (configIpv4['dns']['servers'] as List).cast<Map<String, dynamic>>();
    ...
    final serversIpv6 = (configIpv6['dns']['servers'] as List).cast<Map<String, dynamic>>();
  ...
  test('Test 2: dns.servers does not contain dns-fakeip when disabled', () {
    ...
    final servers = (config['dns']['servers'] as List).cast<Map<String, dynamic>>();
    expect(servers.any((s) => s['tag'] == 'dns-fakeip'), isFalse);
  });

  test('Test 3: dns.rules routes A/AAAA queries to dns-fakeip at the end of rules', () {
    final config = createBuilder(dnsFakeIpEnabled: true).build();
    final dnsRules = (config['dns']['rules'] as List).cast<Map<String, dynamic>>();
  ...
  test('Test 4: route.rules contains resolve action before geoip whitelist/direct rule', () {
    final config = createBuilder(
      dnsFakeIpEnabled: true,
      useRussiaRouteData: true,
    ).build();

    final routeRules = (config['route']['rules'] as List).cast<Map<String, dynamic>>();
  ...
  test('Test 5: route.rules does not contain resolve action when fakeip is disabled', () {
    final config = createBuilder(
  ...
  test('Test 6: experimental.cache_file contains store_fakeip only when fakeip is enabled', () {
  ...
  test('Test 7: isolated fakeip without russiaRouteData or adBlock still generates dns.rules with dns-fakeip', () {
    final config = createBuilder(
      dnsFakeIpEnabled: true,
