...
class SingboxRuntime {
  ...
  Future<void> ensureExecutable(String path) async {
    if (path.trim().isEmpty) {
      throw const FormatException('Binary path is empty.');
    }
    if (!Platform.isAndroid) {
      return;
    }
    try {
      await _withMethodChannelFallback(
        () => _hostApi.ensureExecutable(path),
        () => _methods.invokeMethod<void>('ensureExecutable', {'path': path}),
      );
    } on MissingPluginException {
      throw UnsupportedError('Android platform bridge is unavailable.');
    }
  }

  Future<Map<String, dynamic>> getSnowtunModuleStatus({
    String splitName = 'snowtun',
    String nativeLibraryName = 'libsnowtun.so',
  }) async {
    if (!Platform.isAndroid) {
      return const {};
    }
    try {
      final value = await _withMethodChannelFallback<Map<String, dynamic>>(
        () async => _normalizeMap(
          await _hostApi.getSnowtunModuleStatus(splitName, nativeLibraryName),
        ),
        () async =>
            await _methods.invokeMapMethod<String, dynamic>(
              'getSnowtunModuleStatus',
              {'splitName': splitName, 'nativeLibraryName': nativeLibraryName},
            ) ??
            const {},
      );
      return value;
  ...
  Future<void> installSnowtunModule({
    required String apkPath,
    String expectedPackageName = 'com.etonify.meow_client',
    String splitName = 'snowtun',
  }) async {
    if (!Platform.isAndroid) {
      throw UnsupportedError(
        'Snowtun split installs are only supported on Android.',
      );
    }
    await _withMethodChannelFallback(
      () => _hostApi.installSnowtunModule(
        apkPath,
        expectedPackageName,
        splitName,
      ),
      () => _methods.invokeMethod<void>('installSnowtunModule', {
        'apkPath': apkPath,
  ...
  Future<void> removeSnowtunModule({String splitName = 'snowtun'}) async {
    if (!Platform.isAndroid) {
      return;
    }
    try {
      await _withMethodChannelFallback(
        () => _hostApi.removeSnowtunModule(splitName),
        () => _methods.invokeMethod<void>('removeSnowtunModule', {
          'splitName': splitName,
        }),
      );
    } on MissingPluginException {
  ...
  Future<void> requestInstallPackagesPermission() async {
    ...
    try {
      await _withMethodChannelFallback(
        () => _hostApi.requestInstallPackagesPermission(),
        () => _methods.invokeMethod<void>('requestInstallPackagesPermission'),
      );
    } on MissingPluginException {
      throw UnsupportedError('Android platform bridge is unavailable.');
    }
