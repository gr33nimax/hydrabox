import 'dart:convert';
import 'dart:typed_data';

import 'package:asn1lib/asn1lib.dart';
import 'package:meow_client/data/subscription/happ_crypt5_local.dart';
import 'package:meow_client/models/subscription.dart';
import 'package:pointycastle/export.dart';

const happLatestUserAgent = 'Happ/3.16.1';

class HappCryptoLinkException implements Exception {
  const HappCryptoLinkException(this.message);

  final String message;

  @override
  String toString() => message;
}

class UnsupportedHappCryptoLinkException extends HappCryptoLinkException {
  const UnsupportedHappCryptoLinkException(super.message);
}

class HappCryptoPreparedImport {
  const HappCryptoPreparedImport({
    required this.resolvedUrl,
    required this.requestInfo,
  });

  final String resolvedUrl;
  final SubscriptionInfo? requestInfo;
}

class HappCryptoLinkDecoder {
  HappCryptoLinkDecoder._();

  // Legacy Android/native Happ path, intentionally disabled. Crypt5/5.1 is
  // decrypted locally from extracted selector/key tables in happ_crypt5_local.dart.
  // static const _crypt5Methods = MethodChannel('meow_client/happ_crypto');

  static const _cryptPrefix = 'happ://crypt/';
  static const _crypt2Prefix = 'happ://crypt2/';
  static const _crypt3Prefix = 'happ://crypt3/';
  static const _crypt4Prefix = 'happ://crypt4/';
  static const _crypt5Prefix = 'happ://crypt5/';

  static bool isSupportedLink(String value) {
    final normalized = value.trim().toLowerCase();
    return normalized.startsWith(_cryptPrefix) ||
        normalized.startsWith(_crypt2Prefix) ||
        normalized.startsWith(_crypt3Prefix) ||
        normalized.startsWith(_crypt4Prefix) ||
        normalized.startsWith(_crypt5Prefix);
  }

  static bool isUnsupportedCrypt5Link(String value) {
    return false;
  }

  static bool isAnyHappCryptoLink(String value) {
    final normalized = value.trim().toLowerCase();
    return isSupportedLink(normalized) || normalized.startsWith(_crypt5Prefix);
  }

  static bool isSupportedSubscriptionUrl(String value) {
    final uri = Uri.tryParse(value.trim());
    if (uri == null || !uri.hasScheme) {
      return false;
    }
    return uri.scheme.startsWith('http') || isAnyHappCryptoLink(value);
  }

  static SubscriptionInfo happRequestInfo({String? happCryptoLink}) {
    return SubscriptionInfo(
      happCryptoLink: happCryptoLink,
      customUserAgent: happLatestUserAgent,
      requireHwid: true,
    );
  }

  static Future<HappCryptoPreparedImport> prepare(String link) async {
    final normalized = link.trim();
    if (isUnsupportedCrypt5Link(normalized)) {
      throw const UnsupportedHappCryptoLinkException(
        'Happ crypt5 links are not supported yet.',
      );
    }
    if (!isSupportedLink(normalized)) {
      return HappCryptoPreparedImport(
        resolvedUrl: normalized,
        requestInfo: null,
      );
    }

    final decryptedUrl = await decrypt(normalized);
    final decryptedUri = Uri.tryParse(decryptedUrl);
    if (decryptedUri == null ||
        !decryptedUri.hasScheme ||
        !decryptedUri.scheme.startsWith('http')) {
      throw const HappCryptoLinkException(
        'The Happ link was decrypted, but the resulting URL is invalid.',
      );
    }

    return HappCryptoPreparedImport(
      resolvedUrl: decryptedUrl,
      requestInfo: happRequestInfo(happCryptoLink: normalized),
    );
  }

  static Future<String> decrypt(String encryptedText) async {
    final match = _match(encryptedText.trim());
    if (match == null) {
      throw const HappCryptoLinkException('Unsupported Happ crypto link.');
    }
    if (match.prefix == _crypt5Prefix) {
      return _decryptCrypt5(encryptedText.trim());
    }

    try {
      final encryptedBytes = _decodePayload(
        encryptedText.trim().substring(match.prefix.length),
      );
      final cipher = PKCS1Encoding(RSAEngine())
        ..init(
          false,
          PrivateKeyParameter<RSAPrivateKey>(
            _parsePkcs1PrivateKey(match.keyPem),
          ),
        );
      final decryptedBytes = cipher.process(encryptedBytes);
      return utf8.decode(decryptedBytes).trim();
    } on UnsupportedHappCryptoLinkException {
      rethrow;
    } catch (error) {
      throw HappCryptoLinkException(
        'Failed to decrypt Happ crypto link: $error',
      );
    }
  }

  static _HappCryptoKeyMatch? _match(String input) {
    if (input.startsWith(_crypt4Prefix)) {
      return const _HappCryptoKeyMatch(_crypt4Prefix, _key4Pem);
    }
    if (input.startsWith(_crypt3Prefix)) {
      return const _HappCryptoKeyMatch(_crypt3Prefix, _key3Pem);
    }
    if (input.startsWith(_crypt2Prefix)) {
      return const _HappCryptoKeyMatch(_crypt2Prefix, _key2Pem);
    }
    if (input.startsWith(_cryptPrefix)) {
      return const _HappCryptoKeyMatch(_cryptPrefix, _key1Pem);
    }
    if (input.startsWith(_crypt5Prefix)) {
      return const _HappCryptoKeyMatch(_crypt5Prefix, '');
    }
    return null;
  }

  static Uint8List _decodePayload(String input) {
    final normalized = input
        .replaceAll('-', '+')
        .replaceAll('_', '/')
        .replaceAll('\n', '')
        .trim();
    final padding = normalized.length % 4;
    final padded = padding == 0
        ? normalized
        : normalized.padRight(normalized.length + (4 - padding), '=');
    return Uint8List.fromList(base64.decode(padded));
  }

  static RSAPrivateKey _parsePkcs1PrivateKey(String pem) {
    final body = pem
        .replaceAll('-----BEGIN RSA PRIVATE KEY-----', '')
        .replaceAll('-----END RSA PRIVATE KEY-----', '')
        .replaceAll(RegExp(r'\s'), '');
    final derBytes = base64.decode(body);
    final sequence = ASN1Parser(derBytes).nextObject() as ASN1Sequence;
    final elements = sequence.elements;
    if (elements.length < 9) {
      throw const HappCryptoLinkException('Invalid Happ RSA key format.');
    }

    final modulus = (elements[1] as ASN1Integer).valueAsBigInteger;
    final privateExponent = (elements[3] as ASN1Integer).valueAsBigInteger;
    final p = (elements[4] as ASN1Integer).valueAsBigInteger;
    final q = (elements[5] as ASN1Integer).valueAsBigInteger;
    return RSAPrivateKey(modulus, privateExponent, p, q);
  }

  static Future<String> _decryptCrypt5(String encryptedText) async {
    // Legacy Android/native Happ implementation kept here for reference:
    //
    // final decryptedUrl = await _crypt5Methods.invokeMethod<String>(
    //   'decodeCrypt5',
    //   {'link': encryptedText},
    // );
    // final resolved = decryptedUrl?.trim() ?? '';
    //
    // The active path below is pure Dart and does not depend on liberror-code.so.
    try {
      final resolved = (await HappCrypt5Local.decrypt(encryptedText)).trim();
      if (resolved.isEmpty) {
        throw const HappCryptoLinkException(
          'The Happ crypt5 link was decrypted, but the resulting URL is empty.',
        );
      }
      return resolved;
    } on HappCryptoLinkException {
      rethrow;
    } catch (error) {
      throw HappCryptoLinkException(
        'Failed to decrypt Happ crypt5 link: $error',
      );
    }
  }
}

class _HappCryptoKeyMatch {
  const _HappCryptoKeyMatch(this.prefix, this.keyPem);

  final String prefix;
  final String keyPem;
}

const _key1Pem = '''-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----''';

const _key2Pem = '''-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----''';

const _key3Pem = '''-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----''';

const _key4Pem = '''-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----''';
