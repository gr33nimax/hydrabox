/// Runtime validity policy for HydraBox subscription envelopes.
///
...
class HydraBoxSubscriptionTimePolicy {
  const HydraBoxSubscriptionTimePolicy._();
  ...
  static void validate(Map<String, dynamic> metadata, {DateTime? now}) {
    ...
    if (currentTime.isBefore(notBefore.subtract(allowedClockSkew))) {
      throw const FormatException('HydraBox subscription is not valid yet');
    }
    if (expiresAt != null &&
        !currentTime.isBefore(expiresAt.add(allowedClockSkew))) {
      throw const FormatException('HydraBox subscription has expired');
    }
  }
  ...
  static DateTime _parseRequiredTimestamp(
    ...
    if (parsed == null) {
      throw FormatException('HydraBox $field timestamp is required');
    }
    return parsed;
  }
  ...
  static DateTime? _parseOptionalTimestamp(
    ...
    if (parsed == null) {
      throw FormatException('HydraBox $field timestamp is invalid');
    }
    return parsed.toUtc();
  }
}
