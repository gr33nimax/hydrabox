...
abstract class HydraBoxBasePlatformInterface(
    ...
    override fun findConnectionOwner(
        ...
        try {
            ...
            HydraBoxDiagnostics.log("HydraBoxPlatform", "getConnectionOwnerUid failed", error)
            throw error
        }
    }
    ...
    override fun localDNSTransport(): LocalDNSTransport? = HydraBoxLocalResolver
...
class HydraBoxVpnPlatformInterface(
    private val service: VpnService,
) : HydraBoxBasePlatformInterface(service) {
    companion object {
        @Volatile
    ...
    override fun autoDetectInterfaceControl(fd: Int) {
        val protected = runCatching { service.protect(fd) }.getOrElse { error ->
            HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "protect fd=$fd failed", error)
            throw error
        }
        if (!protected) {
            val error = IllegalStateException("VpnService.protect returned false for fd=$fd")
            HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "protect fd=$fd returned false", error)
            throw error
        }
    }
    ...
    override fun openTun(options: TunOptions): Int {
        ...
        if (options.autoRoute) {
            ...
            if (dnsAddress.isNotEmpty()) {
                ...
                HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "openTun dns=$dnsAddress")
            }
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                if (inet4RouteAddress.hasNext()) {
                    ...
                    HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "openTun add default IPv4 route")
                }
                ...
                if (inet6RouteAddress.hasNext()) {
                    ...
                    builder.addRoute("::", 0)
                    HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "openTun add default IPv6 route")
                }
        ...
        HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "openTun establish()")
        val pfd = runCatching {
            builder.establish() ?: error("android: the application is not prepared or is revoked")
        }.onFailure {
            HydraBoxDiagnostics.log("HydraBoxVpnPlatform", "openTun establish failed", it)
        }.getOrThrow()
        val fd = pfd.detachFd()
...
private object HydraBoxLocalResolver : LocalDNSTransport {
    ...
    override fun exchange(ctx: ExchangeContext, message: ByteArray) {
        ...
        val network = runCatching { HydraBoxDefaultNetworkMonitor.require() }.getOrElse {
            HydraBoxDiagnostics.log(
                "HydraBoxLocalResolver",
                "exchange raw require failed current=${HydraBoxDefaultNetworkMonitor.describeCurrentState()}",
                it,
            )
        ...
        HydraBoxDiagnostics.log(
            "HydraBoxLocalResolver",
            "exchange raw bytes=${message.size} active=${HydraBoxDefaultNetworkMonitor.describeNetwork(network)}",
        )
        ...
        val callback = object : DnsResolver.Callback<ByteArray> {
            override fun onAnswer(answer: ByteArray, rcode: Int) {
                ...
                HydraBoxDiagnostics.log(
                    "HydraBoxLocalResolver",
                    "exchange raw answer rcode=$rcode bytes=${answer.size} active=${HydraBoxDefaultNetworkMonitor.describeNetwork(network)}",
                )
                if (rcode == 0) {
            ...
            override fun onError(error: DnsResolver.DnsException) {
                ...
                HydraBoxDiagnostics.log(
                    "HydraBoxLocalResolver",
                    "exchange raw error active=${HydraBoxDefaultNetworkMonitor.describeNetwork(network)}",
                    error,
                )
        ...
        if (!latch.await(15, TimeUnit.SECONDS) && completed.compareAndSet(false, true)) {
            ...
            HydraBoxDiagnostics.log(
                "HydraBoxLocalResolver",
                "exchange raw timeout active=${HydraBoxDefaultNetworkMonitor.describeNetwork(network)}",
            )
    ...
    override fun lookup(ctx: ExchangeContext, network: String?, domain: String?) {
        ...
        val active = runCatching { HydraBoxDefaultNetworkMonitor.require() }.getOrElse {
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
                runCatching {
        ...
        HydraBoxDiagnostics.log(
            "HydraBoxLocalResolver",
            "lookup dispatch host=$host queryType=${queryType ?: -1} active=${HydraBoxDefaultNetworkMonitor.describeNetwork(active)}",
        )
        if (queryType != null) {
        ...
        if (!latch.await(15, TimeUnit.SECONDS) && completed.compareAndSet(false, true)) {
            ...
            HydraBoxDiagnostics.log(
                "HydraBoxLocalResolver",
                "lookup timeout host=$host active=${HydraBoxDefaultNetworkMonitor.describeNetwork(active)}",
            )
            ctx.errnoCode(OsConstants.ETIMEDOUT)
        }
    }
